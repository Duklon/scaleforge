  ...
  return (
    <div className={styles.card}>
      ...
      <div className={styles.levelRow}>
        <div className={styles.levelName} style={{ color:currentLevel.color }}>Level {currentLevel.level}: {currentLevel.name}</div>
        {nextLevel
          ? <div className={styles.nextLevel}>Next: {nextLevel.name} in {xpToNext} XP</div>
          : <div className={styles.nextLevel} style={{color:'#ffd700'}}>Max Level! 🏆</div>}
      </div>
      <div className={styles.progressBar}>
        <div className={styles.progressFill} style={{ width:`${levelProgress*100}%`, background:`linear-gradient(90deg,${currentLevel.color},${nextLevel?.color||'#ffd700'})` }} />
      </div>
      <div className={styles.todayRow}>
        ...
        {progress.sessionCount >= 3 && <span className={styles.streakBadge}>🔥 On a streak!</span>}
      </div>
    </div>
  )
}
