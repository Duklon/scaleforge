    ...
    <div className={`${styles.container} hide-scrollbar`}>
      <div className={styles.keys}>
        ...
        {octaves.map((oct, octIdx) =>
          Object.entries(BLACK_OFFSETS).map(([note, whiteOffset]) => {
            const totalWhitesBefore = octIdx * 7
            const left = (totalWhitesBefore + whiteOffset) * 42 + 28
            const inScale = notes.includes(note)
            const isRoot = note === root && inScale
            return (
              <div
                key={`${note}${oct}`}
                className={`${styles.blackWrapper}`}
                style={{ left }}
              >
                <div
                  className={`${styles.black} ${isRoot ? styles.rootBlack : inScale ? styles.inScaleBlack : ''}`}
                  onPointerDown={e => {
                    e.stopPropagation()
                    playNote(note, oct, 0.8)
                  }}
